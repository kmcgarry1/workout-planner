<!--
  ...
  Usage:
  <WorkoutBuilder :initial-workout="existingWorkout" @save="handleSave" />
-->
<template>
    ...
    <div :class="selectedExercise ? 'lg:col-span-1' : 'lg:col-span-2'">
      ...
      >
        <CardHeader
          ...
          :subtitle="`${currentWorkout.exercises.length} ${
            currentWorkout.exercises.length === 1 ? 'exercise' : 'exercises'
          }`"
          :divider="true"
          padding="lg"
        >
          <template #prepend>
            <div
              class="p-2.5 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-lg"
            >
        ...
        >
          ...
          <Button variant="danger" size="sm" @click="handleClearWorkout">
            <template #leading>
...
const props = defineProps<WorkoutBuilderProps>();
...
// Initialize with existing workout if provided
onMounted(() => {
  if (props.initialWorkout) {
    const initial = props.initialWorkout;
    currentWorkout.id = initial.id;
    currentWorkout.name = initial.name;
    currentWorkout.date = initial.date;
    currentWorkout.exercises = [...initial.exercises];
    currentWorkout.completed = initial.completed;
    currentWorkout.notes = initial.notes;
    currentWorkout.totalDuration = initial.totalDuration;

    workoutName.value = initial.name;
  }
});

// Watch for changes to initialWorkout prop
watch(
  () => props.initialWorkout,
  (newWorkout) => {
    if (newWorkout) {
      currentWorkout.id = newWorkout.id;
      currentWorkout.name = newWorkout.name;
      currentWorkout.date = newWorkout.date;
      currentWorkout.exercises = [...newWorkout.exercises];
      currentWorkout.completed = newWorkout.completed;
      currentWorkout.notes = newWorkout.notes;
      currentWorkout.totalDuration = newWorkout.totalDuration;

      workoutName.value = newWorkout.name;
    }
  },
  { immediate: true }
);
...
function handleSaveWorkout() {
  ...
  // Emit the save event with the workout data
  emit("save", { ...currentWorkout });
}
</script>
